 ...
 */
public class TileEntityServer extends BaseTile implements INetworkComponent, IInventory, ISidedInventory {
	...
	public void CalculateCables() {
		...
		if (world.getTileEntity(x, y, z) instanceof TileEntityServer) {
	...
	public void calMaxNet() {
		...
		for (int i = 0; i < Contents.length; i++) {
			if (getStackInSlot(i) != null) {
				if (getStackInSlot(i).getItem() instanceof IServerPowerUpgrade) {
					ammoutOfPUps += getStackInSlot(i).stackSize * ((IServerPowerUpgrade) getStackInSlot(i).getItem()).capacity();
				}
			}
		}
	...
	public void chunkTick() {
		if (_ticket == null) {
			_ticket = ForgeChunkManager.requestPlayerTicket(NetworkCore.instance, "network:Server", worldObj, ForgeChunkManager.Type.NORMAL);
			if (_ticket == null) {
