...
public class TileEntityCable extends BaseTile implements INetworkComponent {
	...
	public void updateEntity() {
		...
		for (int i = 0; i < 6; i++) {
			...
			if (curTile == null) {
				...
				if (curTile instanceof TileEntityCable) {
					this.sideCache[i] = 1;
				}
				else {
					this.sideCache[i] = 0;
	...
	public void CalculateCables() {
		...
		// check to see if the server has been removed from the world
						...
						if (Math.abs(x) + Math.abs(y) + Math.abs(z) == 1) {
							...
							if (!visited.contains(target)) {
								...
								if (element.getDepth() < cableMaxLenghth) {
									...
									if (block == NetworkBlocks.server) {
										TileEntity tileEntity = World.getTileEntity(target.getX(), target.getY(), target.getZ());
										if (tileEntity != null && tileEntity instanceof TileEntityServer) {
