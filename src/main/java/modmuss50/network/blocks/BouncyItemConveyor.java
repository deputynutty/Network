...
public class BouncyItemConveyor extends Block {

	public BouncyItemConveyor() {
		super(Material.iron);
		setBlockName("network:BouncyItemConveyor");
		setBlockTextureName("network:BouncyItemConveyor");
	...
	public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity) {
		if(par5Entity instanceof EntityItem || par5Entity instanceof EntityPlayer){} else {
            return;
        }
		...
		if ((par5Entity != null) && (par5Entity.posY > par3 + 0.01D) && (!par5Entity.isSneaking())) {
			if ((par5Entity instanceof EntityItem)) {
				...
				if ((par5Entity instanceof EntityItem)) {
					...
					((EntityItem) par5Entity).delayBeforeCanPickup = 130;
			...
			if (par5Entity instanceof EntityPlayer) {
				if (((EntityPlayer) par5Entity).getDisplayName().contains("MAPCraftGaming")) {
					par5Entity.motionY += (float) par1World.rand.nextGaussian() * 0.05F + 1.0F;
				}
				else {
					par5Entity.motionY += (float) par1World.rand.nextGaussian() * 0.05F + 1.0F;
				}
	...
	public void registerBlockIcons(IIconRegister icon) {
		tops[0] = icon.registerIcon("network:BItemConvayor_0");
		tops[1] = icon.registerIcon("network:BItemConvayor_1");
		tops[2] = icon.registerIcon("network:BItemConvayor_2");
		tops[3] = icon.registerIcon("network:BItemConvayor_3");
