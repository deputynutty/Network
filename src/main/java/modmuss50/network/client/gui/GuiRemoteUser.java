	...
	public void actionPerformed(GuiButton par0Button) {
		...
		for (int i = 0; i < tile.remotetiles.size(); i++) {
			if (par0Button.id == i) {
				...
				NetworkCore.packetPipeline.sendToAll(new PacketRemoteGui(tile.remotetiles.get(i).xCoord, tile.remotetiles.get(i).yCoord, tile.remotetiles.get(i).zCoord, Minecraft.getMinecraft().thePlayer.getDisplayName()));
