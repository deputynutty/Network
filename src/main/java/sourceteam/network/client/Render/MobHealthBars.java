...
public class MobHealthBars {
    ...
    public void renderHealthBar(EntityLivingBase entity, float partialTicks, Entity viewPoint) {
        ...
        if (stack == null) {
            ...
            if (stack.getItem() != NetworkItems.wifiGoggles) {
                return;
            }
        }
        ...
        float scale = 0.046666672F;
        int maxHealth = new BigDecimal(entity.getMaxHealth()).round(new MathContext(2)).intValue();
        int health = new BigDecimal(entity.getHealth()).round(new MathContext(2)).intValue();
        renderLabel(entity, String.format("%s/%s (%s", health, maxHealth, maxHealth == 0 ? 0 : (int) (health * 100 / maxHealth)) + "%)", (float) (x - RenderManager.renderPosX), (float) (y - RenderManager.renderPosY + entity.height + 1.1), (float) (z - RenderManager.renderPosZ), 20);
        GL11.glPushMatrix();
        ...
        GL11.glDisable(GL11.GL_DEPTH_TEST);
        GL11.glDisable(GL11.GL_TEXTURE_2D);
        drawDoubleOutlinedBox(-(int) maxHealth / 2, -1, 3, (int) maxHealth, 2, 0x1A1A1A, 0x969AA5);
        drawSolidGradientRect(-(int) maxHealth + (maxHealth % 2 == 1 ? 1 : 0), -2, 3, (int) health * 2, 4, 0x4a0251, 0x4a0251);
        GL11.glEnable(GL11.GL_TEXTURE_2D);
        GL11.glEnable(GL11.GL_DEPTH_TEST);
        GL11.glDepthMask(true);
        GL11.glEnable(GL11.GL_LIGHTING);
    ...
    public void drawDoubleOutlinedBox(final int x, final int y, final int z, final int width, final int height, final int color, final int outlineColor) {
        drawDoubleOutlinedBox(x, y, z, width, height, color, outlineColor, color);
    }

    public void drawDoubleOutlinedBox(final int x, final int y, final int z, final int width, final int height, final int color, final int outlineColor, final int outline2Color) {
        drawSolidRect(x * 2 - 2, y * 2 - 2, z, (x + width) * 2 + 2, (y + height) * 2 + 2, color);
        drawSolidRect(x * 2 - 1, y * 2 - 1, z, (x + width) * 2 + 1, (y + height) * 2 + 1, outlineColor);
    ...
    public void drawSolidGradientRect(final int x, final int y, final int z, final int width, final int height, final int color1, final int color2) {
        drawSolidGradientRect0(x, y, (x + width), (y + height), color1, color2, z);
    }

    public void drawSolidGradientRect0(final int vertex1, final int vertex2, final int vertex3, final int vertex4, final int color1, final int color2, final int z) {
        GL11.glPushMatrix();
